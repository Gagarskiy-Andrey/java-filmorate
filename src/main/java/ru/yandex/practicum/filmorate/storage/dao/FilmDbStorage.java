...
public class FilmDbStorage implements FilmStorage {
    ...
    private Set<Genre> getGenresByFilmId(Long filmId) {
        String sql = "SELECT g.genre_id, g.name " +
                ...
                "WHERE fg.film_id = ?";
        return new LinkedHashSet<>(Objects.requireNonNull(jdbcTemplate.query(sql, filmMapper::mapToGenres, filmId)));
    }
    ...
    public List<Film> getFilmsList() {
        ...
        List<Film> films = jdbcTemplate.query(sqlFilms, (rs, rowNum) -> filmMapper.mapToFilm(rs));

        Map<Long, Set<Genre>> filmGenresMap = jdbcTemplate.query(sqlGenres, rs -> {
    ...
    public Film update(Film film) {
        ...
        if (get(film.getId()) == null) {
            throw new NotFoundException("Фильм не найден. Ошибка обнавления");
        }
    ...
    public List<Film> getPopularFilms(int count) {
        String sql = "SELECT f.film_id, f.name, f.description, f.releaseDate, f.duration, r.rating_id, " +
                ...
                "ORDER BY likes DESC " +
                "LIMIT ?";
        List<Film> films = jdbcTemplate.query(sql, (rs, rowNum) -> filmMapper.mapToFilm(rs), count);
        ...
        Map<Long, Set<Genre>> genresByFilmId = jdbcTemplate.query(genresByFilmsSql, rs -> {
